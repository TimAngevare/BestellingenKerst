...
def bestellingen(request):
    if request.method == "POST":
        ...
        if bestel_form.is_valid():
            ...
            resultaten = mm.zoek_best({'bestelnr': num, 'telnr': tel, 'dagophalen': dag, 'naam': naam})
            totaal = resultaten[0]
...
def producten(request):
    if request.method == "POST":
        ...
        if producten_form.is_valid():
            ...
            resultaten = mm.zoek_prod({'cat': typ_prod, 'product': prod, 'state': state})
            totaal = resultaten[0]
...
def alles_result(request):
    ...
    if request.method == "POST":
        for key in request.POST.keys():
            if key.startswith('voltooi-'):
                bestel_nmr = key[8:]
                mm.update_state_best(int(bestel_nmr), 'voltooid')
            elif key.startswith('probleem-'):
                bestel_nmr = key[9:]
                mm.update_state_best(int(bestel_nmr), 'probleem')
            elif key.startswith('behandeling-'):
                bestel_nmr = key[12:]
                print("hola " + bestel_nmr)
                mm.update_state_best(int(bestel_nmr), 'bezig')
                return redirect('/zoek/alles?dag=&state=bezig&nmr=' + bestel_nmr)
    resultaten = mm.zoek_best_alles({'dagophalen': dag, 'state': state})
    totaal = resultaten[0]
